...
int main(){
    ...
    while((ch=getchar())!=EOF){
        if(ch == '\t' || ch == ' ' || ch == '\n'){
            if(state == IN_WORD){
                // The previous character was part of a word, the current character
                // is a blank, tab or newline, so we come out of the word.
                // So reset the wordLength variable to 0.
    ...
    for(int i = 0; i < MAX_WORD_SIZE; i++){
        ...
        if(wordLengths[i] < minWordLength)
            minWordLength = wordLengths[i];
    }
    ...
    for(int i = 0; i < MAX_WORD_SIZE; i++){
        if(wordLengths[i] != 0){
            ...
            // For example number of words with length two is 3 and the scale is 4
            // then division results in 0.
            printf("%d:",i);
