...
// NOTE: The program reports no error even if the above line (with a ) ) is uncommented.
};
        ...
        switch(state)
        {
            case PROGRAM:
                ...
                else if(ch == '{' || ch == '(' || ch == '[')
                {
                    ...
                    {
                        printf("%s\n", "Maximum nesting level supported by the program exceeded");
                        return -1;
                    }
                    stack[++stackPtr] = ch;
                }
                else if(ch == '}' || ch == ')' || ch == ']')
                {
                    ...
                    else
                    {
                        ...
                        {
                            printf("Line %d: Syntax error - Opening parenthesis '%c' does not match closing parenthesis '%c'\n", l, stack[stackPtr], ch);
                        }
                    }
                }
                break;
