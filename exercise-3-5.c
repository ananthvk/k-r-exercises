...
int itob(int n, char s[], int b, int destSize)
{
    ...
    // I have use a do-while loop instead of while loop to handle the case when n is 0.
    // So that the loop runs atleast once and digit becomes 0 (0 % anything = 0)
    do
    {
...
int main()
{
    ...
    // The following way of initializing mapping does not work on non ascii systems.
    // This only works on ascii based systems.
    // To make this program portable, it is necessary to hardcode the characters
    // for(int i = '0'; i <= '9'; i++) mapping[j++] = i;
    // for(int i = 'a'; i <= 'z'; i++) mapping[j++] = i;
    // assert(j==36);
