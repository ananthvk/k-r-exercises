...
int lstrindex(char s[], char t[])
{
    ...
    for(i = 0; s[i] != '\0'; i++)
    {
        ...
        for(k = 0; t[k] != '\0'; k++)
        {
            ...
            // if the last character is compared, i+k will exceed the length of the original array.
            ...
            if(s[i+k] == '\0')
            {
                // If the k th char of the source string is the null character, we have reached the end of the string.
                // This will probably get handled in the below condition, but just to be explicit and safe....
                break;
            }
            if(s[i + k] != t[k])
                break;
...
{
    ASSERT_EQ(rstrindex("The quick brown fox jumps over the lazy dogs", "over"), 26);
    ...
    ASSERT_EQ(rstrindex("Once upon a time in in in in in in in in a a a galaxy", "in"), 38);
    ASSERT_EQ(rstrindex("Once upon a time in in in in in in in in a a a galaxy", "a"), 50);
    ASSERT_EQ(rstrindex("Once upon a time in in in in in in in in a a a galaxy", "Oncee"), -1);
    ASSERT_EQ(rstrindex("Once upon a time in in in in in in in in a a a galaxy", "n "), 39);
}
